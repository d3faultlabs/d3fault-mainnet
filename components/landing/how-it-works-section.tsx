...
const steps = [
  {
    ...
    description: "Create a new wallet with 12-word BIP39 seed encrypted with AES-256-GCM using your password. Encryption happens entirely in your browser.",
    code: `// Generate wallet locally
...
// Your seed never leaves this device`,
  },
  {
    ...
    title: "Swap",
    subtitle: "privately",
    description: "Fund a throwaway wallet from your main wallet. The throwaway executes the swap via Jupiter. Output returns to main wallet. Jito protects MEV on mainnet.",
    code: `// Private swap flow
const throwaway = new Keypair()
main.transfer(amount, throwaway)
await jupiter.swap(
  throwaway,
  inputToken,
  outputToken
)
throwaway.transfer(output, main)`,
  },
  {
    number: "03",
    title: "Send",
    subtitle: "via claim links",
    description: "Create a secure claim link with commitment-nullifier cryptography. Recipient claims via relayer. Both identities hidden. Expiry: 1h, 24h, 7d, or never.",
    code: `// Create claim link
const secret = generateSecret()
const commitment = sha256(secret)
const claimLink = 
  'https://d3fault.app/claim#' + secret
// Share link, keep secret safe`,
  },
];
        ...
        <div className="relative mb-0 lg:mb-0 grid lg:grid-cols-2 gap-4 lg:gap-12 items-end">
          ...
          <div className="overflow-hidden pb-0 lg:pb-32">
            ...
            <h2 className={`text-6xl md:text-7xl lg:text-[128px] font-display tracking-tight leading-[0.85] transition-all duration-1000 delay-100 ${
              ...
              <span className="block text-white/30">Private.</span>
              <span className="block text-white/10">Non-custodial.</span>
            </h2>
          </div>
