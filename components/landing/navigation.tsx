      ...
      >
        ...
        >
          ...
          <a href="#" className="flex items-center gap-2 group">
            <span className={`font-display tracking-tight transition-all duration-500 ${isScrolled ? "text-xl text-foreground" : "text-2xl text-white"}`}>D3FAULT</span>
            <span className={`font-mono transition-all duration-500 ${isScrolled ? "text-[10px] mt-0.5 text-muted-foreground" : "text-xs mt-1 text-white/60"}`}>Private DEX</span>
          </a>
          ...
          <div className="hidden md:flex items-center gap-4">
            ...
            <div className="relative">
              <Button
                size="sm"
                className={`rounded-full transition-all duration-500 relative ${isScrolled ? "bg-foreground hover:bg-foreground/90 text-background px-4 h-8 text-xs" : "bg-white hover:bg-white/90 text-black px-6"}`}
                onMouseEnter={() => setIsHoveringLaunch(true)}
                onMouseLeave={() => setIsHoveringLaunch(false)}
              >
                <span className={`transition-all duration-300 ${isHoveringLaunch ? "opacity-0 absolute" : "opacity-100"}`}>
                  Launch App
                </span>
                <span className={`transition-all duration-300 flex items-center gap-2 ${isHoveringLaunch ? "opacity-100" : "opacity-0 absolute"}`}>
                  Coming Soon
                  <svg
      ...
      >
        <div className="flex flex-col h-full px-8 pt-28 pb-8">
          ...
          >
            ...
            <div className="relative">
              <Button
                className={`flex-1 rounded-full h-14 text-base relative transition-all duration-500 ${isScrolled ? "bg-foreground text-background hover:bg-foreground/90" : "bg-foreground text-background"}`}
                onMouseEnter={() => setIsHoveringLaunch(true)}
                onMouseLeave={() => setIsHoveringLaunch(false)}
                onClick={() => setIsMobileMenuOpen(false)}
              >
